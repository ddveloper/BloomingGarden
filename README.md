...
- Debug captures default to viewport-only (no full-page resize), which avoids Chromium shrink/restore flicker during bot actions.
...
If you explicitly need full-page debug images (may cause brief resize/flicker):
```bash
python3 web_bot_crazygames.py --debug-capture-full-page
```
