...
class BotConfig:
    ...
    def load(path: Path) -> "BotConfig":
        ...
        return BotConfig(
            ...
            debug_capture_full_page=bool(raw.get("debug_capture_full_page", False)),
...
class CrazyGamesBot:
    ...
    def _capture_debug_step(self, page: Page, step: int) -> None:
        ...
        if self.config.debug_capture_full_page:
            page.screenshot(path=str(path), full_page=True)
        else:
            vp = page.viewport_size or {"width": 1600, "height": 1000}
            page.screenshot(path=str(path), clip={"x": 0, "y": 0, "width": vp["width"], "height": vp["height"]})
...
def main() -> None:
    ...
    parser.add_argument("--debug-capture-full-page", action="store_true", help="capture full-page debug screenshots (may cause brief visual resize/flicker)")
    ...
    if args.debug_capture_full_page:
        cfg.debug_capture_full_page = True
